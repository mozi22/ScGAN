...
def generator(image_pair):
    ...
    with tf.variable_scope('predict_flow2'):
        ...
        predict_flow = _predict_flow(concat1)

    return predict_flow4, predict_flow

    # predict_flow = change_nans_to_zeros(predict_flow2)
def generator_old(image_pair):
...
def discriminator(input, is_train, reuse=False):
    with tf.variable_scope('discriminator') as scope:
        ...
        conv0 = convrelu2(name='conv0', inputs=input, filters=64, kernel_size=5, stride=2,activation=myLeakyRelu)
        ...
        logits = tf.nn.sigmoid(conv0_r)

        # dcgan
        return logits, conv0_r #, acted_out
